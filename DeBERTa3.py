...
def encode_with_spans(batch):
    try:
        ...
        for i, (tags, offsets) in enumerate(zip(batch["ner_tags"], encoded["offset_mapping"])):
            # Calculate actual spans based on NUM_SPANS limit
            attention_mask_length = len(encoded["attention_mask"][i])
            total_spans = 0
            for j in range(attention_mask_length):
                total_spans += min(attention_mask_length - j, NUM_SPANS)

            # Initialize span mask with exact dimensions needed
            span_mask = np.zeros(total_spans, dtype=np.float32)
                ...
                if tag_str.startswith("B-"):
                    if current_entity is not None:
                        ...
                        if start < attention_mask_length and end < attention_mask_length:
                            ...
                            span_index = 0
                            for s in range(attention_mask_length):
                                for e in range(s, min(attention_mask_length, s + NUM_SPANS)):
                                    if s == start and e == end:
                                        span_mask[span_index] = 1
                                    span_index += 1
...
class SpanNER(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        try:
            ...
            # Calculate actual number of spans based on NUM_SPANS limit
            # This reduces memory usage significantly
            total_spans = 0
            for i in range(seq_len):
                total_spans += min(seq_len - i, NUM_SPANS)

            # Initialize span representations with exact size needed
...
try:
    ...
    total_steps = len(dataset["train"]) // BATCH_SIZE * NUM_EPOCHS // GRADIENT_ACCUMULATION_STEPS
...
for epoch in range(NUM_EPOCHS):
  ...
  # Evaluation
    ...
    with torch.no_grad():
        for batch in dataset["validation"].iter(BATCH_SIZE):
            input_ids = batch["input_ids"].to(device)
            attention_mask = batch["attention_mask"].to(device)
            span_labels = batch["span_labels"].cpu().numpy()

            logits, _ = model(input_ids, attention_mask)
            # CORRECTED: Binary predictions with threshold
            batch_preds = (logits > 0).float().cpu().numpy().astype(int)

            # Process each sample's spans
            for i in range(batch_preds.shape[0]):
                valid_spans = span_labels[i] != -100
    ...
    # CORRECTED: Use binary target names
    logger.info("\nClassification Report:")
    logger.info(classification_report(
        true_labels,
        preds,
        target_names=['non-entity', 'entity'],  # Binary target names
        zero_division=0
    ))
